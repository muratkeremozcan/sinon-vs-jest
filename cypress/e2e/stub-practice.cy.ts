...
describe("stub basics: onCall(), onFirstCall(), onSecondCall(), .returns(..), throws(...), invoke('restore')", () => {
  ...
  it('Return different dynamic values: s.onCall(k).returns(..) ', () => {
    ...
    cy.window().then(w => {
      ...
      const s = cy.stub(w, 'prompt').as('color-stub')
  ...
  it('Restore the original method from a stub alias', () => {
    ...
    cy.stub(person, 'getName').as('getNameStub').returns('Cliff')
    expect(person.getName()).to.eq('Cliff')
...
describe.only('fail if there is a console.log', () => {
  ...
  it('should check at the end of the test', () => {
    ...
    cy.get('@log')
      ...
      .each(call =>
        // @ts-expect-error: chai-friendly
        call.args.map((arg: {to: {not: {contain: (arg0: string) => any}}}) =>
